...
<xsl:stylesheet version="1.0" 
  ...
  -->
    <!-- <xsl:variable name="ErrLogCat" select="'DevErrCat'"/>
        ...
        <xsl:when test="not($wssec/dsig:Signature)">
          ...
          <xsl:message dp:priority="info" dp:type="{$dpconfig:OnErrorLogCat}" >***No signature was included in the WS-Security section of the message.</xsl:message>
            <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
            	select="concat($operationName,',','The ', name($wssec), ' element lacks a signature!An XML digital signature required.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',x509,No signature was included in the WS-Security section of the message')" /> 
            </xsl:message>
        ...
        <xsl:otherwise>
          ...
          <xsl:choose>
            <xsl:when test="$validationResult = '*ASN.1 parse of certificate failed*'">
              ...
              <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
            		select="concat($operationName,',','Your x509 digital certficate is invalid.It has been corrupted or otherwise compromised.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',x509,Certificate failed validation because it was not in the correct format from the client or may have been truncated by another system')" /> 
              </xsl:message>
              <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            </xsl:when>
			...
			<xsl:when test="$validationResult = 'certificate not trusted'">
		      ...
		      <xsl:message dp:priority="info"  dp:type="{$dpconfig:OnErrorLogCat}">***Certificate is not included in Valcred.  Add certificate to Valcred to resolve error.</xsl:message>
                      <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
            			select="concat($operationName,',','One of the signatures on the request document, although valid, was signed by an unknown or untrusted party. The signature cannot be trusted.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',x509,Certificate is not included in Valcred.Add certificate to Valcred to resolve error')" /> 
            	      </xsl:message>
            	      <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            </xsl:when>
			...
			<xsl:when test="$validationResult = 'certificate has expired'">
		      ...
		      <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		             	select="concat($operationName,',','Your x509 certficate has expired.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',x509,Certificate has expired. Add new certificate to Valcred in Datapower to allow access')" /> 
            	      </xsl:message>
            	      <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            </xsl:when>
			...
			<xsl:when test="$validationResult != ''">
              ...
              <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
	                  select="concat($operationName,',','Unknown x509 Error.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',x509,Certificate validation did not fail with a known message.')" /> 
              </xsl:message>
              <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            </xsl:when>
            <xsl:otherwise>
          ...
          <!-- We can infer that the problem is a bad uid/password combination. -->
				...
				<xsl:otherwise>
					...
					<xsl:message dp:priority="info" dp:type="{$dpconfig:OnErrorLogCat}">***The Username/Password in the WS-Security Header was found and LDAP was contacted. The client is most likely not sending a correct combination.  <xsl:value-of select="$LDAPReachability" /></xsl:message>
              				<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
	                  			select="concat($operationName,',','The userid/password is invalid.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',LDAP,The Username/Password in the WS-Security Header was found and LDAP was contacted. The client is most likely not sending a correct combination.')" /> 
              				</xsl:message>
              				<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
				</xsl:otherwise>
                </xsl:choose>
      ...
      <xsl:choose>
          ...
          <!-- We can infer that the problem is a bad uid/password combination. -->
				...
				<xsl:otherwise>
					...
					<xsl:message dp:priority="info" dp:type="{$dpconfig:OnErrorLogCat}">***The Username/Password in the BASIC Authentication was found and LDAP was contacted. The client is most likely not sending a correct combination.  <xsl:value-of select="$LDAPReachability" /></xsl:message>
			        	<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
						select="concat($operationName,',','The userid/password combination does not work.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',BasicAuth,The Username/Password in the BASIC Authentication was found and LDAP was contacted.The client is most likely not sending a correct combination')" /> 
			        	</xsl:message>				
					<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
				</xsl:otherwise>
                </xsl:choose>
        ...
        <xsl:when test="$uid != '' and $pwd = ''">
          <xsl:value-of select="concat('Missing password for userid: ', $uid)"/>
          	...
          	<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
			select="concat($operationName,',','Missing password for userid.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',BasicAuth,Username/Password BASIC Authentication did not include a password value.Client should correct the message.')" /> 
		</xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
        </xsl:when>
        <xsl:otherwise>
          ...
          <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		select="concat($operationName,',','The Basic Auth header is missing a userid or password.,',$cnVal,',',$userID,',',$baVal,',',$soapAct,',',$errCode,',',$errSubCode,',BasicAuth,Username/Password BASIC Authentication was either not provided or it was not found in the correct location.Client should correct the message.')" /> 
	  </xsl:message>
	  <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
        </xsl:otherwise>
      </xsl:choose>
