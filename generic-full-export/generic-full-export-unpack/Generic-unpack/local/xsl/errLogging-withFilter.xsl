...
<xsl:variable name="CounterName" select="concat('/monitor-count/', $ServiceName, 'ErrorMsgCt')"/>
<dp:increment-integer name="$CounterName"/>

 <xsl:message dp:type="{$LogCat}" dp:priority="info">
    <xsl:value-of select=" concat('[',$ServiceName,'] [',$operationName, '] ErrorResponse:')" />
    ...
    <xsl:if test="$DebugVar">
         <xsl:message dp:priority="info" dp:type="{$LogCatDebug}">
            <xsl:value-of select=" concat('[',$ServiceName, '] [',$operationName,'] Request:')"/>
            <xsl:copy-of select="filter:expungeWSSec(filter:obscureCreditCardInfo(dp:variable('var://context/copyOfInput/_roottree')))"/>
        </xsl:message>

        <xsl:message dp:type="{$LogCatDebug}" dp:priority="info">
           <xsl:value-of select=" concat('[',$ServiceName, '] [',$operationName,'] ERROR:')" />
           <!--<xsl:copy-of select="filter:expungeWSSec(.)"/>-->
       <xsl:copy-of select="filter:expungeWSSec(filter:obscureCreditCardInfo((.)))"/>
        </xsl:message>
    </xsl:if>
