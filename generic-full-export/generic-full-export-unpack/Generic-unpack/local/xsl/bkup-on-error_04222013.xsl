...
<!--Defines messages based on ErrorCodes and Subcodes.  Will log appropriate action internally-->
    <xsl:variable name="errorMessage">
      <xsl:choose>
          ...
          <xsl:choose>
            <xsl:when test="$errorDirection='response'">
              <xsl:value-of
                ...
                <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Error parsing or validating BACKEND SERVICE response message.Uncertain what would cause this error.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		...
		<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            ...
            <xsl:otherwise>
              <xsl:value-of
                ...
                <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Error parsing or validating APPLICATION request message.Uncertain what would cause this error.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
          ...
          <xsl:choose>
            <xsl:when test="$errorDirection='response'">
              <xsl:value-of select="concat('Error parsing BACKEND SERVICE response message.','(TID:',$transactionId,')')" />
                <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Error parsing BACKEND SERVICE response message.Uncertain what would cause this error.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Error parsing APPLICATION request message.Uncertain what would cause this error.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Access to this Service Denied by Policy. The operation called was blocked by a reject all action.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            ...
            <xsl:otherwise>
              ...
              <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Generic failure in middleware. Uncertain what would cause this error.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
         ...
         <xsl:when test="$errorCode='0x01130011'">
          ...
          <xsl:message dp:priority="error" dp:type="{$SupportLogCategory}">***Failed to process response headers. The failure usually indicates the proxy was set up for non-ssl and the backend server is expecting SSL. Update the proxy on Datapower to resolve issue.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
	  <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:when test="$errorCode='0x00c3000c'">
          ...
          <xsl:message dp:priority="error" dp:type="{$SupportLogCategory}">***The proxy is not configured properly.  The output of one action does not match the input of the action following it. Modify the communications between the two rule actions to match.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
	  <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
	  ...
	  <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
        ...
        <xsl:when test="$errorCode='0x00c30008'">
           <xsl:choose>
            <xsl:when test="$errorDirection='response'">
              ...
              <xsl:message dp:priority="error" dp:type="{$SupportLogCategory}">***The server did not respond with valid SOAP.  The server most likely sent a response like HTML that is not allowed with a web service proxy.  The server is expected to send valid SOAP as the response.  Try to send test message directly to server to determine what is being sent to Datapower.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
              <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
	      ...
	      <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
            ...
            <xsl:otherwise>
             <xsl:value-of select="concat('Client Message did not include a SOAP Action Header.','(TID:',$transactionId,')')" />
                <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***The web service requires a SOAP Action Header.  The client must include a SOAP Action header.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
		<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
		...
		<xsl:when test="$errorCode='0x01130006' or $errorCode='0x01130007' or $errorCode='0x0113001e' or $errorCode='0x01130009' or $errorCode='0x00c3000f' or $errorCode='0x00030004'">
          ...
          <xsl:message dp:priority="error" dp:type="{$SupportLogCategory}">***Failed to establish a backside connection. Something has interfered with connectivity to backend server.  Test that backend server can be pinged, or called directly.<xsl:value-of select="concat('(TID:',$transactionId,')')"/></xsl:message>
	<xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
     ...
     <xsl:choose>
      ...
      <xsl:when test="local:getId($input) = ''">
        <xsl:choose>
		  <!--<xsl:when test="$valcred != '' ">-->
		   <!--values are: aaaX509, aaaUserid, aaaSkip and aaaReject-->
            ...
            <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***The control file is configured for a username/password.  The message had no security information in the message.  The client must authenticate with the appropriate type for this service call.</xsl:message>
	    <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
	    ...
	    <xsl:message dp:priority="info" dp:type="{$ErrReqCat}"><xsl:value-of select="$reqBody" /> </xsl:message>
         ...
         <xsl:value-of select="'Authentication Failure:  SOAP body must be signed for signature-based authentication. No credentials were provided.  Contact IT,SOASvsSupport.'"/> 
         <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***The control file is configured for a X509 certificate.  The message had no security information in the message.  The client must authenticate with the appropriate type for this service call.</xsl:message>
         <xsl:message dp:priority="info" dp:type="{$ErrLogCat}"><xsl:value-of 
         ...
         <xsl:when test="$aaa_type='aaaReject'">
			  <xsl:value-of select="'Authentication Reject:  The service is configured to reject traffic.'"/> 
	          <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***The control file is configured to Reject everyone for this operation in the service.</xsl:message>
         </xsl:when>
         
         <xsl:otherwise>
                   <xsl:value-of select="'Authentication Skip:  The service is configured to skip AAA processing.'"/> 
                   <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***The control file is configured to SKIP AAA for this operation in the service.  No AAA error should occur.</xsl:message>
         </xsl:otherwise> 
         </xsl:choose>    
       </xsl:when>
      <xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="local:isIdX509($input)">
            ...
            <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Certificate details were examined with isIdX509 Function. This is the DN from the certificate: <xsl:value-of select="$dnVal" /></xsl:message>
          </xsl:when>
          <xsl:when test="local:isIdUsernameToken($input)">
            ...
            <xsl:value-of select="local:examineUsernameToken($wssec)"/>
              <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Username/Password token were examinded with the examineUsernameToken function.  This is the result from the function: <xsl:value-of select="local:examineUsernameToken($wssec)" /></xsl:message>
          </xsl:when>
          <xsl:when test="local:isIdBasicAuth()">
            ...
            <xsl:value-of select="local:examineBasicAuth()"/>
              <xsl:message dp:priority="info" dp:type="{$SupportLogCategory}">***Username/Password from the HTTP Basic Auth were examined by the examineBasicAuth function.  This is the result from the function: <xsl:value-of select="local:examineBasicAuth()" /></xsl:message>
          </xsl:when>
          <xsl:otherwise>            
            <xsl:message dp:priority="error" dp:type="{$SupportLogCategory}">***Someone added a new type of user credential without updating the code in on-error.xsl.</xsl:message>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
    </xsl:choose>
