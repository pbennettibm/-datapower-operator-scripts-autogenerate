...
<xsl:choose>
	<xsl:when test="dp:responding()">
			...
			<xsl:variable name="DmdCde2" select="dp:variable('var://context/ActiveMatrixESB/DmdCde2')"/>
			<xsl:variable name="OSSCde2" select="dp:variable('var://context/ActiveMatrixESB/OSSCde2')"/>
			<xsl:variable name="AppID2" select="dp:variable('var://context/ActiveMatrixESB/OSSCde2')"/>
			<xsl:variable name="AppChNm2" select="dp:variable('var://context/ActiveMatrixESB/AppChNm2')"/>
			<xsl:variable name="SOATransID2" select="dp:variable('var://context/ActiveMatrixESB/AppChNm2')"/>
	
      		<xsl:message dp:type="{$LogCat2}" dp:priority="info">|<xsl:value-of select="$ServiceName" />|<xsl:value-of select="$OperationName" />|<xsl:value-of select="$ClientIP" />|<xsl:value-of select="$AAAType" />	|<xsl:value-of select="$OU" />|<xsl:value-of select="$Uid" />|<xsl:value-of select="$ResponseComplete" />|<xsl:value-of select="$ElapsedTime" />|<xsl:value-of select="$proxyURI" />|<xsl:value-of select="$PartnerCall" />|<xsl:value-of select="$RequestSize" />|<xsl:value-of select="$ResponseSize" />|<xsl:value-of select="$DmdCde2" />|<xsl:value-of select="$OSSCde2" />|<xsl:value-of select="$AppID2" />|<xsl:value-of select="$AppChNm2" />|<xsl:value-of select="$SOATransID2" /></xsl:message>
	...
	<dp:set-variable name="'var://context/ActiveMatrixESB/DmdCde2'" value="$DmdCde"/>
	<dp:set-variable name="'var://context/ActiveMatrixESB/OSSCde2'" value="$OSSCde"/>
	<dp:set-variable name="'var://context/ActiveMatrixESB/AppID2'" value="$AppID"/>
	<dp:set-variable name="'var://context/ActiveMatrixESB/AppChNm2'" value="$AppChNm"/>
	<dp:set-variable name="'var://context/ActiveMatrixESB/SOATransID2'" value="$SOATransID"/>
	
		<xsl:message dp:type="{$LogCat}" dp:priority="info">|<xsl:value-of select="$ServiceName" />|<xsl:value-of select="$OperationName" />|<xsl:value-of select="$ClientIP" />|<xsl:value-of select="$AAAType" />	|<xsl:value-of select="$OU" />|<xsl:value-of select="$Uid" />|<xsl:value-of select="$ResponseComplete" />|<xsl:value-of select="$ElapsedTime" />|<xsl:value-of select="$proxyURI" />|<xsl:value-of select="$PartnerCall" />|<xsl:value-of select="$RequestSize" />|<xsl:value-of select="$ResponseSize" />|<xsl:value-of select="$DmdCde" />|<xsl:value-of select="$OSSCde" />|<xsl:value-of select="$AppID" />|<xsl:value-of select="$AppChNm" />|<xsl:value-of select="$SOATransID" /></xsl:message>
