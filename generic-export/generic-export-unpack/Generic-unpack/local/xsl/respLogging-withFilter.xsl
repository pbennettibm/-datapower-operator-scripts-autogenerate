...
<xsl:variable name="ServiceName" select="dp:variable('var://context/ActiveMatrixESB/serviceName')"/>
 ...
 <xsl:message dp:type="{$LogCat}" dp:priority="error">
    <xsl:value-of select=" concat('[',$ServiceName,'] [',$operationName, '] NonSoapResponse:')" />
    ...
    <xsl:if test="$DebugVar">
       <xsl:message dp:type="{$LogCatDebug}" dp:priority="error">
          <xsl:value-of select=" concat('[',$ServiceName,'] [',$operationName, '] NonSoapResponse:')" />
          <xsl:copy-of select="filter:expungeWSSec(.)"/>
       </xsl:message>
    </xsl:if>
