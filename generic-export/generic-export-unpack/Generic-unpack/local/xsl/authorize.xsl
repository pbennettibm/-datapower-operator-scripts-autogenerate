  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="intersectionOfRoles">
      ...
      <xsl:choose>
	    ...
	    <!-- This was added to provide access to all users of the  08/17/2016-->
            <!-- <xsl:if test="'EliSoaDatapowerUsers' = $role]">
              <xsl:value-of select="'.'"/>
              <xsl:message>### match the roleEliSoaDatapowerUsers</xsl:message>
            </xsl:if> -->
          </xsl:for-each>
