         ...
         <!-- <xsl:variable name="cert" select="string($wssec/wsse:BinarySecurityToken[@ValueType='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3'])"/>-->
          ...
          <xsl:choose>
            <xsl:when test="$validationResult = '*ASN.1 parse of certificate failed*'">
              <xsl:variable name="errorMessage" select="concat('The x509 digital certficate is invalid. It has been corrupted or otherwise compromised.(TID:',$transactionId,')')"/>
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
			...
			<xsl:when test="$validationResult = 'certificate not trusted'">
               <xsl:variable name="errorMessage" select="concat('One of the signatures on the request document was signed by an unknown or untrusted party. The signature cannot be trusted.(TID:',$transactionId,')')"/>
				<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />		      
			...
			<xsl:when test="$validationResult = 'certificate has expired'">
               <xsl:variable name="errorMessage" select="concat('The x509 certficate has expired.(TID:',$transactionId,')')"/>
			   <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
            ...
            <xsl:otherwise>
              ...
              <xsl:variable name="errorMessage" select="concat('The digital signature in the message is invalid.  Either the message was changed after it was signed or tampered with in transit.(TID:',$transactionId,')')"/>
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />           
