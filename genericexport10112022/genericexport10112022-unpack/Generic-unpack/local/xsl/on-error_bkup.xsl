...
<!--Defines messages based on ErrorCodes and Subcodes. -->
      <xsl:choose>
         <xsl:when test="$errorCode='0x00230001'">
          <xsl:choose>
            <xsl:when test="$errorDirection='response'">
                <xsl:variable name="errorMessage" select="'Error parsing or validating BACKEND SERVICE response message.'"/>
            ...
            <xsl:otherwise>
              <xsl:variable name="errorMessage" select="'Error parsing or validating APPLICATION(client)request message.'"/>
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
        ...
        <xsl:when test="$errorCode='0x00030001'">
          <xsl:choose>
            <xsl:when test="$errorDirection='response'">
                <xsl:variable name="errorMessage" select="'Error parsing BACKEND SERVICE response message.'" />
				<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
            ...
            <xsl:otherwise>
				  <xsl:variable name="errorMessage" select="'Error parsing APPLICATION(client) request message.'" />
				  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
        ...
        <xsl:when test="$errorCode='0x00d30003'">
          <xsl:choose>
			...
			<xsl:variable name="errorMessage" select="'Access to this Service Denied by Policy.'" />
			<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
         ...
         <xsl:when test="$errorSubCode='0x01d30002'">
            ...
            <xsl:otherwise>
              <xsl:variable name="errorMessage" select="'Unrecognized failure in SOA. Please contact IT SOASvsSupport.'" />
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
         ...
         <xsl:when test="$errorCode='0x01130011'">
          <xsl:variable name="errorMessage" select="'Failed to process response headers from backend server.'" />
		  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
	...
	<xsl:when test="$errorCode='0x00c3000c'">
          <xsl:variable name="errorMessage" select="'Proxy is misconfigured.'" />
			<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
     ...
     <xsl:when test="$errorCode='0x00c30008'">
           <xsl:choose>
            <xsl:when test="$errorDirection='response'">
	      <xsl:variable name="errorMessage" select="'Remote server message was not valid XML. Verify SOAP response is being sent by backend server.'" />
			<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
           ...
           <xsl:otherwise>
             <xsl:variable name="errorMessage" select="'Client Message did not include a SOAP Action Header.'" />
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
	...
	<xsl:when test="$errorCode='0x01130006' or $errorCode='0x01130007' or $errorCode='0x0113001e' or $errorCode='0x01130009' or $errorCode='0x00c3000f' or $errorCode='0x00030004'">
          <xsl:variable name="errorMessage" select="'Failed to establish a backside connection.'" />
			<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
...
<xsl:choose>
      <xsl:when test="local:getId($input) = ''">
        <xsl:choose>
		   <!--values are: aaaX509, aaaUserid, aaaSkip and aaaReject-->
            ...
            <xsl:when test="$aaa_type='aaaUserid'">
            <xsl:variable name="errorMessage" select="'Authentication Failure:  SOAP Message should have included a username/password combination. No credentials were provided.  Contact IT SOASvsSupport.'"/>        
            <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
         ...
         <xsl:variable name="errorMessage" select="'Authentication Failure:  SOAP body must be signed for signature-based authentication. No credentials were provided.  Contact IT SOASvsSupport.'"/>
		 <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
         ...
         <xsl:when test="$aaa_type='aaaReject'">
			  <xsl:variable name="errorMessage" select="'Authentication Reject:  The service is configured to reject traffic.'"/> 
			  <dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
         ...
         <xsl:otherwise>
                   <xsl:variable name="errorMessage" select="'Authentication Skip:  The service is configured to skip AAA processing.'"/> 
					<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
			...
			<xsl:variable name="certInfo" select="local:getId($input)"/>
			 ...
			 <xsl:choose>
			    <xsl:when test="$certInfo='OTHER_X509_Value'">
					<xsl:variable name="errorMessage" select="'Authentication Error: WS-Security Header encountered an error.'"/> 
					<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />         
          ...
          <xsl:otherwise>   
			<xsl:variable name="errorMessage" select="'Authentication Not Supported: The authentication type provided is not supported.'"/> 
			<dp:set-variable name="'var://context/errorInfo/errorMessage'" value="$errorMessage" />         
